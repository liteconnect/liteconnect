...
*Setup instructions for a Gitian build of Liteconnect Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Liteconnect
...
- [Building Liteconnect Core](#building-liteconnect-core)
...
Clone the git repositories for liteconnect and Gitian.
...
in the liteconnect repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Liteconnect Core
----------------

To build Liteconnect Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the liteconnect repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/liteconnect/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/liteconnect-project/liteconnect
...
For example:
```bash
URL=https://github.com/thrasher-/liteconnect.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit liteconnect=${COMMIT} --url liteconnect=${URL} ../liteconnect/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit liteconnect=${COMMIT} --url liteconnect=${URL} ../liteconnect/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit liteconnect=${COMMIT} --url liteconnect=${URL} ../liteconnect/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the liteconnect git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../liteconnect/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/liteconnect
SIGPATH=/some/root/path/liteconnect-detached-sigs

./bin/gbuild --url liteconnect=${BTCPATH},signature=${SIGPATH} ../liteconnect/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/liteconnect-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/liteconnect-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/liteconnect-osx-build.assert
```
...
[liteconnect-project/gitian.sigs.ltc](https://github.com/liteconnect-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
